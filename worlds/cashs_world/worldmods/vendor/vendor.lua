...
-- Typo fixed by JDC in many places:
-- Places a space between the currency amount and the currency type when showing how
-- much the machine buys or sells an item
...
-- Checks 1 space in each of x,y, and z directions for one of vendor.traversable_node_types
-- Does not check in diagonal directions
-- Returns false if node is nil or not one of vendor.traversable_node_types
-- Returns true if node is one of vendor.traversable_node_types
...
-- BUG
-- The original function did not allow vending machines to be placed side-by-side
-- Modified by JDC so that the checks are only vertical not in either of the horizontal directions
-- This is a quick hack and could be written better to allow side-by-side usage
...
vendor.find_chest_inv = function(owner, pos, dx, dy, dz, nodename, amount, removing)
	...
	--node.name == "custom:chest" or
	if ( node.name == "custom:chest_locked") then
		local meta = minetest.env:get_meta(pos)
		if ( node.name == "custom:chest_locked" and owner ~= meta:get_string("owner") ) then
			return nil
		end
