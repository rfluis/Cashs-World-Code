...
["custom:stone"]=4,["custom:tree"]=2,["custom:jungletree"]=2,["custom:pinetree"]=2,["custom:acacia_tree"]=2,
["custom:lava_source"]=21890,["custom:water_source"]=11000,["custom:obsidian"]=20,["bedrock2:bedrock"]=999999};
...
basic_machines.hardness["custom:river_water_source"]=21890.;
...
basic_machines.dig_up_table = {["custom:cactus"]=true,["custom:tree"]=true,["custom:jungletree"]=true,["custom:pinetree"]=true,
["custom:acacia_tree"]=true,["custom:papyrus"]=true};

-- set up nodes for harvest when digging: [nodename] = {what remains after harvest, harvest result}
basic_machines.harvest_table = {
["mese_crystals:mese_crystal_ore4"] = {"mese_crystals:mese_crystal_ore1", "custom:mese_crystal 3"}, -- harvesting mese crystals
["mese_crystals:mese_crystal_ore3"] = {"mese_crystals:mese_crystal_ore1", "custom:mese_crystal 2"},
["mese_crystals:mese_crystal_ore2"] = {"mese_crystals:mese_crystal_ore1", "custom:mese_crystal 1"},
...
["signs:text"] = true,
["custom:water_source"] = true,
["custom:lava_source"] = true,
["protector:protect"] = true,
["protector:protect2"] = true,
["travelnet:travelnet"] = true,
["custom:chest_locked"] = true,
["custom:protected_chest"] = true,
["custom:lava_source"] = true,
["custom:water_source"] = true,
}
...
minetest.register_node("basic_machines:mover", {
	...
	tiles = {"compass_top.png","custom_furnace_top.png", "basic_machine_mover_side.png","basic_machine_mover_side.png","basic_machine_mover_side.png","basic_machine_mover_side.png"},
	...
	can_dig = function(pos, player) -- dont dig if upgrades inside, cause they will be destroyed
		...
		return not(inv:contains_item("upgrade", ItemStack({name="custom:mese"})));
	end,
	...
	allow_metadata_inventory_put = function(pos, listname, index, stack, player)
		...
		if listname == "upgrade" then
			...
			local upgrade_name = "custom:mese";
			if meta:get_int("elevator")==1 then upgrade_name = "custom:diamondblock" end
			if stack:get_name() == upgrade_name then
			--inv:contains_item("upgrade", ItemStack({name="custom:mese"})) then
	...
	mesecons = {effector = {
		action_on = function (pos, node,ttl)
			...
			if node1.name == "custom:chest_locked" or mode == "inventory" then fuel_cost = basic_machines.hardness[prefer] or 1 end;
		...
		if mode == "object" then -- teleport objects and return
			...
			if node2.name == "custom:chest" or node2.name == "custom:chest_locked" then
				target_chest = true
			end
		...
		local source_chest=false; if string.find(node1.name,"custom:chest") then source_chest=true end
		...
		if target_chest then
			...
			if dig then
				...
				if dig_up == true then -- dig up to 16 nodes
					...
					local r = 1; if node1.name == "custom:cactus" or node1.name == "custom:papyrus" then r = 0 end
...
local function use_keypad(pos,ttl, again) -- position, time to live ( how many times can signal travel before vanishing to prevent infinite recursion )
	...
	if T > 2 then -- overheat
			minetest.sound_play("custom_cool_lava",{pos = pos, max_hear_distance = 16, gain = 0.25})
...
minetest.register_node("basic_machines:detector", {
	...
	after_place_node = function(pos, placer)
		...
		inv:set_stack("mode_select", 1, ItemStack("custom:coal_lump"))
	...
	mesecons = {effector = {
		action_on = function (pos, node,ttl)
			...
			if mode == "node" then
				...
				if node~="" and string.find(tnode,"custom:chest") then -- if source is chest, look inside chest for items
				...
				if op~= "" then
					...
					if node~="" and string.find(tnode,"custom:chest") then -- it source is chest, look inside chest for items
...
minetest.register_chatcommand("clockgen", { -- test: toggle machine running with clockgens
	...
	func = function(name, param)
        -- required to prevent server crash when running server in terminal mode
		local name1 = minetest.get_player_by_name(name)
		if name1 == nil then
            print ("Invalid command. " .. name .. " is not a known player")
            return;
        end
			...
			if punchset[name].pos.x == pos.x and punchset[name].pos.z == pos.z then -- check if elevator mode
				if math.abs(punchset[name].pos.y-pos.y)>10 then -- trying to make elevator?
					...
					if meta:get_string("mode")=="object" then -- only if object mode
						...
						local inv = meta:get_inventory();
						local upgrade = 0;
						if inv:get_stack("upgrade", 1):get_name() == "custom:diamondblock" then
							upgrade = (inv:get_stack("upgrade", 1):get_count()) or 0;
						end
...
minetest.register_on_player_receive_fields(function(player,formname,fields)
	...
	if string.sub(formname,0,string.len(fname)) == fname then
		...
		if fields.help == "help" then
			...
			"For example, to detect empty space write air, to detect tree write custom:tree, to detect ripe wheat write farming:wheat_8, for flowing water write custom:water_flowing ... "..
...
-- minetest.register_craft({
	...
	-- recipe = {
		...
		-- {"custom:mese_crystal", "custom:mese_crystal","custom:mese_crystal"},
		-- {"custom:stone", "basic_machines:keypad", "custom:stone"}
	-- }
-- })
