...
-- Setting this to false stops cats from digging water and providing players with water sources.
-- However, doing this is a 'hack'. I really need to disable the digging of water
...
function node_ok(pos, fallback)
	...
	fallback = fallback or "custom:dirt"
			...
			if enable_pathfind_digging then
				...
				if s.y < p1.y then
					...
					if not minetest.is_protected(s, "") then
						minetest.set_node(s, {name = "custom:dirt"})
					end
					...
					if not minetest.is_protected(s, "") then
						...
						if node1 ~= "air"
-- Players were getting mobs to generate water_sources. Fixed!
						and node1 ~= "custom:water_source"
						and node1 ~= "custom:lava_source"
...
minetest.register_entity(name, {
	...
	on_step = function(self, dtime)
		...
		elseif self.state == "walk" then
			...
			if self.fly
			and self.fly_in == "custom:water_source"
			and not lp then
	...
	on_punch = function(self, hitter, tflp, tool_capabilities, dir)
		...
		if weapon:get_definition().sounds ~= nil then
			...
			minetest.sound_play("custom_punch", {
				object = hitter,
				max_hear_distance = 5
			})
		end
...
local c_obsidian = minetest.get_content_id("custom:obsidian")
local c_brick = minetest.get_content_id("custom:obsidianbrick")
local c_chest = minetest.get_content_id("custom:chest_locked")
