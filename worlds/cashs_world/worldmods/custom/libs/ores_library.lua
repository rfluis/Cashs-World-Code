...
-- minetest.register.node must be used on items that can be placed
-- into the world either by mapgen or by a player placing the item
...
local function get_recipe(c, name)
 ...
 function add_ore(modname, oredefs)
	local img_base = modname .. "_" .. oredefs.ore_name
	local tool_base = modname .. ":"
	local item_base = tool_base .. oredefs.ore_name
	local lump_item = item_base .. "_lump"

	if oredefs.makes_ore then
		minetest.register_node(modname .. ":mineral_" .. oredefs.ore_name, {
			description = S("%s Ore"):format(S(oredefs.description)),
			tiles = {"default_stone.png^" .. modname .. "_mineral_" .. oredefs.ore_name .. ".png"},
			groups = {cracky = 3},
		...
		if oredefs.makes_lump then
		minetest.register_craftitem(lump_item, {
			description = S("%s Lump"):format(S(oredefs.description)),
