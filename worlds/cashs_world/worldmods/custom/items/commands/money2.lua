...
dofile(minetest.get_modpath("custom") .. "/config_files/money2_config.lua")
dofile(minetest.get_modpath("custom") .. "/items/misc/locked_sign.lua")

money.set = function(name, value)
	local output = io.open(minetest.get_worldpath() .. "/Mod_Data/money" .. "/money_" .. name .. ".txt", "w")
	output:write(value)
	io.close(output)
end

-- Changed credit from undeclared global to declared local - JDC
money.get = function(name)
	local input = io.open(minetest.get_worldpath() .. "/Mod_Data/money" .. "/money_" .. name .. ".txt", "r")
	if not input then
		return nil
	end
...
-- Changed name from undeclared global to declared local - JDC
