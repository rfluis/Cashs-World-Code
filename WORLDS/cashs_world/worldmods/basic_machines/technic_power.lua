...
battery_upgrade = function(pos)
	...
	for i=1,2 do
		...
		if item == "custom:mese" then
			count1=count1+count
		elseif item == "custom:diamondblock" then
			count2=count2+count
		end
	end
...
local machines_activate_furnace = minetest.registered_nodes["custom:furnace"].on_metadata_inventory_put; -- this function will activate furnace
	...
	mesecons = {effector = {
		action_on = function (pos, node,ttl)
			...
			if energy>=1 then -- need at least 1 energy
				...
				if node== "custom:furnace" or node=="custom:furnace_active" then
					...
					if fuel_time>40 or fuel_totaltime == 0 or node=="custom:furnace" then -- must burn for at least 40 secs or furnace out of fuel
						...
						if node~="custom:furnace_active" and machines_activate_furnace then machines_activate_furnace(pos) end
...
minetest.register_node("basic_machines:generator", {
	...
	after_place_node = function(pos, placer)
		...
		on_receive_fields = function(pos, formname, fields, sender)
			if fields.quit then return end
			if fields.help then
				local text = "Generators slowly produce power crystals that can be used to recharge batteries.\n\nUpgrading the generator with gold and diamond blocks will:\n1 - Increase the rate at which the crystals are produced.\n2 - Allow other forms of power crystals to be produced\n\nThe power crystals comes in three forms:\nUpgrade Level       Output           Provides\nLow (0-19)            Power Cell     1 power\nMedium (20-98)    Power Block  11 power\nHigh (99)              Power Rod     100 power\n\nWhere (number) is the number of gold blocks AND diamond blocks required.\n\nPress the REFRESH button to .......\n\nYou can automate the process of recharging by using a mover in inventory mode, taking from inventory \"fuel\".\nWARNING: Using an improperly configured mover on a generator will destroy any gold and diamond blocks in the upgrade slots!";
...
-- minetest.register_craft({
	...
	-- recipe = {
		-- {"","custom:steel_ingot",""},
		-- {"custom:steel_ingot","custom:mese","custom:steel_ingot"},
		-- {"","custom:diamond",""},
...
-- minetest.register_craft({
	...
	-- recipe = {
		-- {"","",""},
		-- {"custom:diamondblock","basic_machines:battery","custom:diamondblock"},
		-- {"custom:diamondblock","custom:diamondblock","custom:diamondblock"}
