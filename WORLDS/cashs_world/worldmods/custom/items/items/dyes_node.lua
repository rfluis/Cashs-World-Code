...
for _, row in ipairs(dyes) do
	...
	local groups = row[3]
	local item_name = "custom:dye_" .. name
	local item_image = "custom_dye_" .. name .. ".png"
...
for one, results in pairs(mixes) do
	for i, result in ipairs(results) do
		...
		minetest.register_craft({
			type = "shapeless",
			output = 'custom:dye_' .. result .. ' 2',
			recipe = {'custom:dye_' .. one, 'custom:dye_' .. another},
		})
	end
end
