...
signs_lib.enable_colored_metal_signs = true
...
signs_lib.wallmounted_rotate = function(pos, node, user, mode, new_param2)
	...
	for _, v in ipairs(minetest.get_objects_inside_radius(pos, 0.5)) do
		...
		if e and e.name == "custom:signs_text" then
			v:remove()
		end
	end
...
signs_lib.sign_node_list = {
	"custom:sign_wall_wood",
	"custom:sign_wall_steel",
	"custom:sign_yard",
	"custom:sign_hanging",
	"custom:sign_wall_green",
	"custom:sign_wall_yellow",
	"custom:sign_wall_red",
	"custom:sign_wall_white_red",
	"custom:sign_wall_white_black",
	"custom:sign_wall_orange",
	"custom:sign_wall_blue",
	"custom:sign_wall_brown",
	"custom:sign_wall_locked"
}

local default_sign, default_sign_image

-- Default sign was renamed in 0.4.14. Support both & old versions.
--if minetest.registered_nodes["custom:sign_wall_wood"] then
	signs_lib.default_sign = "custom:sign_wall_wood"
	signs_lib.default_sign_image = "custom_sign_wood.png"
--else
--	signs_lib.default_sign = "custom:sign_wall"
--	signs_lib.default_sign_image = "custom_sign_wall.png"
--end

signs_lib.default_sign_metal = "custom:sign_wall_steel"
signs_lib.default_sign_metal_image = "custom_sign_steel.png"
...
local MP = minetest.get_modpath("custom")
...
signs_lib.update_sign = function(pos, fields, owner)
	...
	elseif signnode.name == "custom:sign_yard" then
		sign_info = signs_lib.yard_sign_model.textpos[minetest.get_node(pos).param2 + 1]
	elseif signnode.name == "custom:sign_hanging" then
	...
	elseif string.find(signnode.name, "sign_wall") then
		if signnode.name == signs_lib.default_sign
		  or signnode.name == signs_lib.default_sign_metal
		  or signnode.name == "custom:sign_wall_locked" then
	...
	local text = minetest.add_entity({x = pos.x + sign_info.delta.x,
										...
										z = pos.z + sign_info.delta.z}, "custom:signs_text")
...
function signs_lib.determine_sign_type(itemstack, placer, pointed_thing, locked)
	...
	if minetest.registered_nodes[node.name] and minetest.registered_nodes[node.name].on_rightclick then
		...
		if fences_with_sign[pt_name] and signname == signs_lib.default_sign then
			minetest.add_node(under, {name = fences_with_sign[pt_name], param2 = fdir})
		elseif wdir == 0 and signname == signs_lib.default_sign then
			minetest.add_node(above, {name = "custom:sign_hanging", param2 = fdir})
		elseif wdir == 1 and signname == signs_lib.default_sign then
			minetest.add_node(above, {name = "custom:sign_yard", param2 = fdir})
		elseif signname == signs_lib.default_sign_metal then
			minetest.add_node(above, {name = signname, param2 = wdir })
		elseif signname ~= signs_lib.default_sign
		  and signname ~= signs_lib.default_sign_metal
		  and signname ~= "custom:sign_wall_locked" then -- it's a signs_lib colored metal wall sign.
...
signs_lib.signs_text_on_activate = function(self)
